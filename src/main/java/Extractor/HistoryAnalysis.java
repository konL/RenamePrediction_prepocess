...
//        String TraceHistory = "C:\\project\\IdentifierStyle\\log\\dump\\extend\\" + projectname + ".csv";
            ...
            for(Diff onediff:difflist)
            {
                ...
                for(String line:content)
                {
                    ...
                    if(line.startsWith("-")||line.startsWith("- "))
                    {
                        ...
                        System.out.println("curr="+statement);
                        ...
                        if(simi>0.85)
                        {
                            ...
                            System.out.println(statement);
                                ...
                                System.out.println("stmtId="+stmtId.toString());
                                System.out.println("lineId="+lineId.toString());

                            //保存两个标识符是否的确变化了（上面有可能是除了标识符以外的修改）
//                            boolean id_sim=false;
                            float id_simi=0;
                            if(stmtId.size()!=0 && lineId.size()!=0) {
                              id_simi = SimilarityCal.calEditSimi(stmtId.get(0), lineId.get(0));
//                                id_sim = stmtId.get(0).equals(lineId.get(0));


                            }
                            //确定标识符的确变化了 id.sim=false 标识符发生变化
                            if(id_simi!=1.0) {
//                            if(!id_sim) {
